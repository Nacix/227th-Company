...
// player setVariable ["KYT_didMRT", false]; // Used to track when attachments are switched using MRT
KYT_didMRT = false; // Used to track when attachments are switched using MRT

// Fired if _unit's weapon or pointer is switched while their laser is active
["KYT_weaponStateChanged", {
...
["CBA_attachmentSwitched", {
	...
	if (KYT_isLaserOn) then { _unit action ["IRLaserOn", _unit] }; // Re-enables the laser if it was active
	KYT_didMRT = true;
}] call CBA_fnc_AddEventHandler;
...
[
	{
		...
		if (_weapon isNotEqualTo _newWeapon || _pointer isNotEqualTo _newPointer) then {
			if (KYT_didMRT) then { KYT_didMRT = false } else { ["KYT_weaponStateChanged", [player, _newWeapon, _newPointer, _weapon, _pointer]] call CBA_fnc_localEvent };

			_weapon = _newWeapon;
			_pointer = _newPointer;
		} else {
			KYT_isLaserOn = player isIRLaserOn _weapon; // Sends out the current state of our laser pointer
		};
	},
